...
export default function StoryLibrary({ stories, locale }: StoryLibraryProps) {
  ...
  const filteredStories = useMemo(() => {
    const preferredAge = mode === 'child' ? childProfile.ageRange : ageFilter
    return stories.filter((story) => {
      if (freeOnly && !story.isFree) return false
      if (preferredAge !== 'all' && story.ageRange !== preferredAge) return false
      return true
    })
  }, [stories, ageFilter, freeOnly, mode, childProfile.ageRange])
      ...
      <div className="rounded-2xl bg-white/85 p-5 shadow-lg ring-1 ring-[#0f766e]/15">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-xl font-bold text-[#0f172a]">
            {mode === 'parent' ? t('curateStories') : t('storiesFor', { name: childProfile.name })}
          </h2>
